...
void d_eglSetProcName(void *context, const char *name, int size) {
    Render_Thread_Context *thread_context = (Render_Thread_Context *)context;
    Process_Context *process_context = thread_context->process_context;
    if (process_context->guest_proc_name_size != 0) {
        g_free(process_context->guest_proc_name);
    }
    process_context->guest_proc_name = name;
    process_context->guest_proc_name_size = size;
    express_printf("eglSetProcName ctx %p update name %s size %d\n", context, name, size);
 }