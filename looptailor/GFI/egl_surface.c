...
void egl_surface_swap_buffer(void *render_context, Window_Buffer *surface, uint64_t gbuffer_id, int width, int height, int hal_format)
{
    ...
    const char* current_name = get_current_game_name();
    if (thread_context->process_context->guest_proc_name_size != 0 &&
        strcmp(thread_context->process_context->guest_proc_name,
               current_name) == 0) {
        // game rendering, intercept.
        sunshine_encode_from_gbuffer(opengl_context, now_draw_gbuffer);
    }
