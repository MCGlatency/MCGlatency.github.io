...
void BufferStateLayer::onLayerDisplayed(const sp<Fence>& releaseFence) {
    ALOGI("BufferStateLayer onLayerDisplayed(), layer name: %s, time: %" PRId64, getName().c_str(), systemTime());
...
bool BufferStateLayer::setBuffer(const std::shared_ptr<renderengine::ExternalTexture>& buffer,
    ...
    ALOGI("BufferStateLayer setBuffer(), framenumber=%" PRIu64 ", layername: %s, time: %" PRId64, frameNumber, getName().c_str(), systemTime());
...
bool BufferStateLayer::hasFrameUpdate() const {
    ALOGI("BufferStateLayer hasFrameUpdate(), layer name: %s" PRId64, getName().c_str());
