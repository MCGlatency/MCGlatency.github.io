...
#include "egl_timers.h"

#include <cinttypes>

#ifdef LOG_TAG
#undef LOG_TAG
#define LOG_TAG "EGLPLALOG"
#endif
...
EGLBoolean eglSwapBuffersWithDamageKHRImpl(EGLDisplay dpy, EGLSurface draw, EGLint* rects,
    ...
    if (n_rects == 0) {
        ALOGI("EGL egl_platform_entries.cpp call eglSwapBuffers, block time: %" PRId64, systemTime());
        EGLBoolean ret = s->cnx->egl.eglSwapBuffers(dp->disp.dpy, s->surface);
        ALOGI("EGL egl_platform_entries.cpp call eglSwapBuffers, release time: %" PRId64, systemTime());
        return ret;
    }
    ...
    if (s->cnx->egl.eglSwapBuffersWithDamageKHR) {
        ALOGI("EGL egl_platform_entries.cpp call eglSwapBuffersWithDamageKHR, block time: %" PRId64, systemTime());
        EGLBoolean ret = s->cnx->egl.eglSwapBuffersWithDamageKHR(dp->disp.dpy, s->surface, rects, n_rects);
        ALOGI("EGL egl_platform_entries.cpp call eglSwapBuffersWithDamageKHR, release time: %" PRId64, systemTime());
        return ret;
    }
    ALOGI("EGL egl_platform_entries.cpp call eglSwapBuffers, block time: %" PRId64, systemTime());
    EGLBoolean ret = s->cnx->egl.eglSwapBuffers(dp->disp.dpy, s->surface);
