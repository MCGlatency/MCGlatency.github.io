...
#undef _CONVERT
  ...
  encode_avcodec(int64_t frame_nr, avcodec_encode_session_t &session, safe::mail_raw_t::queue_t<packet_t> &packets, void *channel_data, std::optional<std::chrono::steady_clock::time_point> frame_timestamp) {
    ...
    BOOST_LOG(info) << "Frame " << frame->pts << " sent for encoding. Time: " << tv.tv_sec*1000+tv.tv_msec;
    ...
    while (ret >= 0) {
      ...
      struct timeval tv;
      gettimeofday(&tv, NULL);
      BOOST_LOG(info) << "Frame " << frame->pts << " finish encoding. Time: " << tv.tv_sec*1000+tv.tv_msec;
