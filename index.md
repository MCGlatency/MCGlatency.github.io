<!-- # Rethink the VSync in Mobile Cloud Gaming -->
...
Through cloud-side computing and rendering,
    mobile cloud gaming (MCG) is expected to deliver high-quality gaming experiences to budget mobile devices.
However, our experiments on mainstream MCG platforms reveal that even with low network latency, all platforms exhibit a high interactive latency (112 ms -- 403 ms, from a user input action to its display response) that critically affects players' quality of experience.
Surprisingly, we find Vertical Synchronization (VSync, a classic access control mechanism for graphic frames) to be the largest contributor to the delay within the end-to-end graphics pipeline; as many as 5 VSync events are intricately linked with the complex graphics processing logic on both the client and cloud sides.
To address this,
    we design a whole-loop VSync regulator, dubbed LoopTailor,
    which minimizes VSync events by decoupling game rendering from the lengthy cloud-side graphics pipeline and
    synchronizing it directly with the client.
We implement LoopTailor on a commercial MCG platform and commodity Android devices,
    reducing the graphics processing delay by an average of 37% and achieving a stably sub-100 ms interactive latency.
...
We have provided logical demo codes of RVC in this folder.
